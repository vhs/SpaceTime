...
The clock component is AVR-based and communicates over a serial link with a Raspberry Pi, which performs
NTP time sync and updates variables on https://api.vanhack.ca based on the clock's state.
https://isvhsopen.com references the VHS API to display the open state and the closing time.
...
See [Schematic](doc/schematic.pdf) for more information.
...
The [Raspberry Pi's code](python) is written in Python and is described more [here](software/README.md).

The [firmware code](firmware) is written in C for the avr-gcc compiler and is described more [here](firmware/README.md).

See [GitHub Issues](https://github.com/vhs/SpaceTime/issues) for open issues and features.
